...
export function BalanceHistoryChart({ history, locale }: { history: BalanceSnapshot[]; locale: "en" | "vi" }) {
  ...
  return <section className={styles.balanceChart} aria-label={locale === "vi" ? "Lịch sử số dư USDC thực tế" : "Observed USDC balance history"}>
    <div className={styles.chartRanges} aria-label={locale === "vi" ? "Khoảng thời gian" : "History range"}>{ranges.map((item) => <button type="button" key={item} className={range === item ? styles.chartRangeActive : undefined} aria-pressed={range === item} onClick={() => setRange(item)}>{item}</button>)}</div>
    {change !== undefined && <p className={`${styles.chartChange} ${change > 0n ? styles.chartPositive : ""}`}>{change > 0n ? "+" : change < 0n ? "−" : ""}{formatUsdc(change < 0n ? -change : change)} USDC <span>{range}</span></p>}
