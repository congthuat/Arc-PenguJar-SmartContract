...
export function JarCard({ jar }: { jar: Jar }) {
  ...
  const displayName = privateMetadata.metadata?.name || (privateMetadata.isPrivate ? t("jar.privateName") : jar.name);
  ...
  return (
    <article className={`jar-card ${(jar.closed || (displayTarget > 0n && jar.balance >= displayTarget)) ? "jar-card-completed" : ""}`}>
      <div className="jar-card-art" aria-hidden="true"><span>✦</span><i /></div>
      ...
      <div className="security-badges"><span>{jar.mode === 0 ? "SAFE" : "SHIELDED"}</span>{jar.guardian !== "0x0000000000000000000000000000000000000000" && <span>{t("jar.guardianProtected")}</span>}{privateMetadata.isPrivate && <span>{t("jar.privateMetadata")}</span>}{jar.frozen && <span className="danger">{t("jar.frozen")}</span>}</div>
      <div className="jar-card-heading">
        ...
        {privateMetadata.isPrivate && !privateMetadata.metadata && <div className="private-metadata-state"><span>{privateMetadata.available ? t("jar.metadataEncrypted") : t("jar.metadataUnavailable")}</span>{privateMetadata.available && privateMetadata.isOwner && <button onClick={() => void privateMetadata.decrypt()} disabled={privateMetadata.isDecrypting}>{privateMetadata.isDecrypting ? t("jar.waitingSignature") : t("jar.decryptMetadata")}</button>}{privateMetadata.error && <small>{privateMetadata.error}</small>}</div>}
      ...
      <div className="progress-label"><span>{privateMetadata.isPrivate && !privateMetadata.metadata ? t("jar.privateMetadata") : t("jar.percentSaved", { percent: progress.toFixed(progress % 1 ? 1 : 0) })}</span><span>{t("jar.target")} {privateMetadata.isPrivate && !privateMetadata.metadata ? "—" : formatUsdc(displayTarget)} USDC</span></div>
